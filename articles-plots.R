...
saltboxplot <- function(df) {
    ggplot(data = df, aes(x = group, y = NA.)) +
...
pcoaplot <- function(pseq, pcoa) {
    ...
    pcoa.plot <- ggplot(data=pcoa.df, aes(x=Axis.1, y=Axis, color = color)) +
        facet_wrap(~key, scales = "free_y", strip.position = "left", labeller = axis_labeller) +
        geom_point(size = 0.05, data = subset(pcoa.df, color == 0)) +
        geom_point(size = 0.05, data = subset(pcoa.df, color == 1)) +
